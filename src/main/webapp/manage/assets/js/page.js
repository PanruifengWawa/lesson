...
function Page(ele,w,btn,navi){
  ...
  function _init(first){
    ...
    par.findClass("page-arrow").remove();
    par.findClass("page-navi").remove();
    if(_pageNum>1){
      if(btn!=false){
        var _top=(ele.hei()/2-20)+"px";
        ["left","right"].each(function(item){
          par.append(J.new("img.page-arrow.pa-"+item).attr("src","assets/images/"+item+".png").clk(new Function(par.varname+"."+item+"()")).css("top",_top));
        });
      }
      if(navi!=false){
        _navi=J.new("div.page-navi");
        for(var i=0;i<_pageNum;i++){
          var ni=J.new("span.page-navi-i").clk(function(){
