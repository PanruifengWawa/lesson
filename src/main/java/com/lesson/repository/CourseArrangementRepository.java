...
public interface CourseArrangementRepository extends JpaRepository<CourseArrangement, Long> {
	...
	@Query("select arrange from CourseArrangement arrange where arrange.courseId = :courseId order by arrange.arrangementOrder asc,arrange.courseArrangementId asc" )
...
//	
//	@Transactional
//	@Query(value = "update t_course_arrangement set arrangement_order = :arrangementOrder where course_arrangement_id = :courseArrangementId",nativeQuery = true)
//	@Modifying
//	void updateArrangementOrderByCourseArrangementId(@Param("arrangementOrder") Integer arrangementOrder, @Param("courseArrangementId") Long courseArrangementId);
